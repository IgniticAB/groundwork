...
Drop the `groundwork/` directory into a place your AI harness picks up skills from:

- **Claude Code / Cowork**: copy into `.claude/skills/groundwork/` at the project root, or into `~/.claude/skills/groundwork/` to use across all projects.
- **Cursor**: copy into `.cursor/skills/groundwork/` (requires Skills enabled in Cursor Nightly).
- **Codex**: copy into `.agents/skills/groundwork/` (repo) or `~/.agents/skills/groundwork/` (user).

The same directory works in all three; the harness reads the `SKILL.md`.

### Optional: `/gw` slash command

For Claude Code users who want a real slash shortcut, copy `slash-commands/gw.md` into `.claude/commands/gw.md` (at the project root or in your home `~/.claude/commands/`). Then `/gw init`, `/gw audit`, `/gw plan` etc. autocomplete in the slash menu and dispatch to the skill.

Without this step, `/gw` still works as a trigger phrase (the skill descriptions match on it), but it is not a true Claude Code slash command and may not autocomplete. Other harnesses (Cursor, Codex) use their own slash mechanisms; the skill itself triggers via natural language regardless.

## How to use

Once installed, type the skill name or one of its commands in your harness:

- `groundwork init` — bootstrap this repo.
- `groundwork audit` — score the current setup.
- `groundwork plan` — draft a plan before coding the next change.

With the optional slash shim installed, the same commands work as `/gw init`, `/gw audit`, `/gw plan`.
...
A companion CLI lives in `cli/` and is published separately as `@ignitic/groundwork` on npm. It runs deterministic, LLM-free anti-pattern checks: stale CLAUDE.md, MCP credential leaks, oversized context files, placeholder comments, common secrets, license headers, and ADR drift.

```bash
npx @ignitic/groundwork detect           # one-shot scan
npx @ignitic/groundwork detect --json    # CI-ready output
npx @ignitic/groundwork watch            # re-detect on file changes
npx @ignitic/groundwork list-rules       # show all rules
```
