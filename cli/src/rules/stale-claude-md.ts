// Detects CLAUDE.md / AGENTS.md that mention a package manager not declared in package.json,
// or a Node framework not present in dependencies.
import type { Rule, Finding } from '../types.js';

const FRAMEWORKS = ['next', 'remix', 'svelte', 'sveltekit', 'astro', 'nuxt', 'vue', 'react', 'angular'];
const PKG_MGRS = ['npm', 'pnpm', 'yarn', 'bun'];

export const staleClaudeMd: Rule = {
  id: 'stale-claude-md',
  ...
  async run(ctx) {
    ...
    if (!pkgRaw) return findings; // Not a Node project; skip.

    let pkg: { packageManager?: string; dependencies?: Record<string, string>; devDependencies?: Record<string, string> };
    try {
    ...
    for (const target of ['CLAUDE.md', 'AGENTS.md', '.github/copilot-instructions.md']) {
      ...
      if (declaredPkgMgr) {
        for (const mgr of PKG_MGRS) {
          if (mgr === declaredPkgMgr) continue;
