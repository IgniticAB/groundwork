...
export function isIgnored(body: string, oneIndexedLine: number): boolean {
  const lines = body.split('\n');
  if (oneIndexedLine < 1 || oneIndexedLine > lines.length) return false;
...
export function isFileIgnored(body: string): boolean {
